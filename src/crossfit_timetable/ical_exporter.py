...
class ICalExporter:
    ...
    def generate_ical_content(self, classes: List[ClassItem]) -> bytes:
        ...
        for class_item in classes:
            ...
            event.add('description', f"CrossFit Class\nCoach: {class_item.coach}\nSource: {class_item.source_url}")
            # Add unique UID for Google Calendar
            event.add('uid', f"{class_item.date.isoformat()}-{class_item.event_name.replace(' ', '-')}-{class_item.coach.replace(' ', '-')}-crossfit-timetable")
